...
<!-- <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/4.5.2/css/bootstrap.css"> -->
   ...
   <body>
    ...
    <tbody>
      <?php foreach ($breads as $key => $slider): ?>
      <tr>
        <td>{{$key+1}}</td>
        <td>{{$slider->title}}</td>
      ...
      <td>
        <img src="{{asset('uploads/Bread_photos')}}/{{$slider->image}}" alt="" width="100">
      </td>
      <td>
        <img src="{{asset('uploads/Bread2_photos')}}/{{$slider->image}}" alt="" width="100">
      </td>
      <td>
        <img src="{{asset('uploads/bread_icon_photos')}}/{{$slider->image}}" alt="" width="100">
      </td>
        <td>
       ...
       <form id="edit-form.{{$slider->id}}" action="{{route('bread.edit',$slider->id)}}" method="POST" style="display:none;">
         @csrf
         @method('GET')
       </form>
       ...
       <form id="delete-form.{{$slider->id}}" action="{{route('bread.destroy',$slider->id)}}" method="POST" style="display:none;">
         @csrf
         @method('DELETE')
       </form>
